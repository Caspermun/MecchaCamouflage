...
namespace
{
    ...
    auto rgb_to_hex(double r, double g, double b) -> std::string
    {
        const int ri = std::min(255, std::max(0, static_cast<int>(r * 255.0 + 0.5)));
        const int gi = std::min(255, std::max(0, static_cast<int>(g * 255.0 + 0.5)));
        const int bi = std::min(255, std::max(0, static_cast<int>(b * 255.0 + 0.5)));
        char buffer[16]{};
        std::snprintf(buffer, sizeof(buffer), "#%02X%02X%02X", ri, gi, bi);
        return buffer;
    }
    ...
    auto paint_payload(const Config& config, const ProcessInfo& process) -> std::string
    {
        ...
        auto payload = std::string("{\"native_apply_mode\":") + json_string(config.native_apply_mode) +
                       ...
                       ",\"front_region_mode\":" + json_string(region_mode_to_string(config.tuning.front_region_mode)) +
                       ",\"side_region_mode\":" + json_string(region_mode_to_string(config.tuning.side_region_mode)) +
                       ",\"back_region_mode\":" + json_string(region_mode_to_string(config.tuning.back_region_mode)) +
                       ",\"enable_front_paint\":" + (config.tuning.front_region_mode == RegionMode::Paint ? "true" : "false") +
                       ",\"enable_side_paint\":" + (config.tuning.side_region_mode == RegionMode::Paint ? "true" : "false") +
                       ",\"enable_back_paint\":" + (config.tuning.back_region_mode == RegionMode::Paint ? "true" : "false") +
                       ",\"auto_material\":" + (config.tuning.auto_material_properties ? "true" : "false") +
                       ...
                       ",\"fill_color\":" + json_string(rgb_to_hex(config.tuning.fill_color_r, config.tuning.fill_color_g, config.tuning.fill_color_b)) +
                       ",\"fill_color_r\":" + std::to_string(config.tuning.fill_color_r) +
    ...
    const auto run = [&]() -> int {
        ...
        if (config.mode == L"shutdown")
        {
            if (config.bridge_port <= 0)
                config.bridge_port = settings.bridge_port;
            if (config.bridge_port <= 0)
            {
